...
class CropAudioActivity :
    BaseActivity(R.layout.activity_crop_audio, R.string.crop_audio_using_time) {
        ...
        when (v?.id) {
            ...
            R.id.btnSelectStartTime -> {
                if (!TextUtils.isEmpty(maxTimeString) && !TextUtils.equals(
                    ...
                    Toast.makeText(
                        this,
                        getString(R.string.input_audio_validate_message),
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
    ...
    override fun selectedFiles(mediaFiles: List<MediaFile>?, requestCode: Int) {
        if (requestCode == Common.AUDIO_FILE_REQUEST_CODE) {
            if (mediaFiles != null && mediaFiles.isNotEmpty()) {
                ...
                Toast.makeText(
                    this,
                    getString(R.string.audio_not_selected_toast_message),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    private fun isSelectedTimeValid(selectedTime: String?): Boolean {
        ...
        try {
            val time1: Date = SimpleDateFormat(
                Common.TIME_FORMAT,
                Locale.ENGLISH
            ).parse(getString(R.string.zero_time))
            val time2: Date =
