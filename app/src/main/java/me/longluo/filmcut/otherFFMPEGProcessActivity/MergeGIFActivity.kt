...
class MergeGIFActivity : BaseActivity(R.layout.activity_merge_gif, R.string.merge_gif) {
            ...
            R.id.btnMerge -> {
                when {
                    !isInputGifSelected -> {
                        Toast.makeText(
                            this,
                            getString(R.string.input_gif_validate_message),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                    ...
                    TextUtils.isEmpty(edtXScale.text.toString()) -> {
                        Toast.makeText(
                            this,
                            getString(R.string.x_width_validation),
                            Toast.LENGTH_SHORT
                        ).show()
                    }

                    edtXScale.text.toString().toFloat() > 100 || edtXScale.text.toString()
                        .toFloat() <= 0 -> {
                        Toast.makeText(
                            this,
                            getString(R.string.x_width_invalid),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                    ...
                    TextUtils.isEmpty(edtYScale.text.toString()) -> {
                        Toast.makeText(
                            this,
                            getString(R.string.y_height_validation),
                            Toast.LENGTH_SHORT
                        ).show()
                    }

                    edtYScale.text.toString().toFloat() > 100 || edtYScale.text.toString()
                        .toFloat() <= 0 -> {
                        Toast.makeText(
                            this,
                            getString(R.string.y_height_invalid),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
    ...
    private fun combineGifProcess() {
        ...
        mediaFiles?.let {
            ...
            val query = ffmpegQueryExtension.mergeGIF(
                pathsList,
                xPos,
                yPos,
                widthScale,
            ...
            Common.IMAGE_FILE_REQUEST_CODE -> {
                if (mediaFiles != null && mediaFiles.isNotEmpty()) {
                    ...
                    if (size == 2 && isGifFile) {
                        ...
                        Toast.makeText(
                            this,
                            getString(R.string.please_selected_minimum_2_gif_file),
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        Toast.makeText(
                            this,
                            getString(R.string.gif_extension_validation),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                } else {
                    Toast.makeText(
                        this,
                        getString(R.string.please_select_gif_file),
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
