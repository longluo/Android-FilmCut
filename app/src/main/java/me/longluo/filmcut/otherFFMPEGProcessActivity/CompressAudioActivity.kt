...
import me.longluo.videooperations.Common.BITRATE_128
import me.longluo.videooperations.FFmpegCallBack
import me.longluo.videooperations.LogMessage

class CompressAudioActivity :
    BaseActivity(R.layout.activity_compress_audio, R.string.compress_audio) {
            ...
            R.id.btnChange -> {
                mediaFiles?.size?.let {
                    if (!isInputAudioSelected) {
                        Toast.makeText(
                            this,
                            getString(R.string.validation_of_audio),
                            Toast.LENGTH_SHORT
                        ).show()
                        return
                    }
    ...
    private fun compressAudioProcess() {
        ...
        val query = ffmpegQueryExtension.compressAudio(
            inputAudioPath = tvInputPathAudio.text.toString(),
            bitrate = BITRATE_128,
            output = outputPath
        )
