...
class CompressVideoActivity :
    BaseActivity(R.layout.activity_compress_video, R.string.compress_a_video) {
        ...
        when (requestCode) {
            Common.VIDEO_FILE_REQUEST_CODE -> {
                if (mediaFiles != null && mediaFiles.isNotEmpty()) {
                    ...
                    inputFileSize.text =
                        "Input file Size : ${Common.getFileSize(File(tvInputPathVideo.text.toString()))}"
                } else {
                    Toast.makeText(
                        this,
    ...
    private fun compressProcess() {
        ...
        CallBackOfQuery().callQuery(query, object : FFmpegCallBack {
            ...
            override fun success() {
                tvOutputPath.text = String.format(
                    getString(R.string.output_path_with_size),
                    outputPath,
                    Common.getFileSize(File(outputPath))
                )
                processStop()
            }
