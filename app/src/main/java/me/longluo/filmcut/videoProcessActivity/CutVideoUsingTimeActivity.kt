...
import me.longluo.videooperations.Common.TIME_FORMAT
import me.longluo.videooperations.Common.VIDEO_FILE_REQUEST_CODE
import me.longluo.videooperations.Common.stringForTime
...
class CutVideoUsingTimeActivity :
    BaseActivity(R.layout.activity_cut_video_using_time, R.string.cut_video_using_time) {
            ...
            R.id.btnConvert -> {
                when {
                    ...
                    TextUtils.isEmpty(startTimeString) -> {
                        Toast.makeText(
                            this,
                            getString(R.string.start_time_validate_message),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                    ...
                    TextUtils.isEmpty(endTimeString) -> {
                        Toast.makeText(
                            this,
                            getString(R.string.end_time_validate_message),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                        ...
                        if (isValidation()) {
                            ...
                            Toast.makeText(
                                this,
                                getString(R.string.start_time_end_time_validation_message),
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
    ...
    private fun isSelectedTimeValid(selectedTime: String?): Boolean {
        ...
        try {
            val time1: Date =
                SimpleDateFormat(TIME_FORMAT, Locale.ENGLISH).parse(getString(R.string.zero_time))
